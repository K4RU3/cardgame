...
export class GameManager {
    ...
    callEvent(event: GameEvent, selfRef: number): [boolean, GameEvent, () => void] {
        if (UNCALLABLE_EVENT.includes(event.type)) {
            console.warn(`${event.type} は呼び出すことができません。`);
    ...
    #runScripts(api: ScriptAPI, game: Game, selfRef: number, phase: 'before' | 'after') {
        ...
        if (!(self instanceof GameObject) || self instanceof Game) { return; }
    ...
    #createScriptAPI(event: GameEvent): ScriptAPI {
        ...
        const baseAPI = {
            type: event.type,
            get game(): Game { return this.unref(gameId) as Game },
            isCanceled: false,
    ...
    createPlayer(arg: {
        state?: stateType, script?: ScriptData[], hp?: number, mp?: number, sanity?: number, inventory?: number[]
    } = {}): Player {
...
export class Script {
    ...
    getScriptsFilteredByType(type: GameEvent['type']): ScriptData[] {
        return JSON.parse(JSON.stringify(
            this.#scripts.filter(script => script.scripts[type])
        ));
    }
    ...
    getScriptByName(name: string): ScriptData['scripts'] {
        return JSON.parse(JSON.stringify(
            this.#scripts.
                find(script => script.name === name)?.scripts
        ));
    }

    changeScript(script: ScriptData): boolean {
        const [isCanceled, updatedEvent, afterScript] = this.#manager.deref()!.callEvent({
            type: 'changeScript',
            value: {
...
export class Game extends GameObject {
    ...
    #calcAllCardWeight(): number {
        return this.#cardsRef
            ...
            .map(card => card.state.copy['weight'] || 0)
            .reduce((a, b) => a + b, 0);
    }
    ...
    weightedRandomClonedCard(): Card | null {
        ...
        const cardsWeightList = this.#cardsRef
            .map((ref) => this.managerRef.deref()?.getById(ref) as Card)
            .map(card => card.state.copy['weight'] || 1);
        ...
        // 二分探索でインデックスを見つける
        let low = 0, high = cumulativeWeights.length - 1;
        while (low < high) {
...
export class Player extends GameObject {
    ...
    #applyEvent(event: 'heal' | 'damage' | 'recharge' | 'discharge' | 'givesanity' | 'takesanity', amount: number, ref: number) {
    ...
    heal(amount: number, ref?: number) {
        this.#applyEvent('heal', amount, ref ?? -1);
    }

    damage(amount: number, ref?: number) {
        this.#applyEvent('damage', amount, ref ?? -1);
    }

    recharge(amount: number, ref?: number) {
        this.#applyEvent('recharge', amount, ref ?? -1);
    }

    discharge(amount: number, ref?: number) {
        this.#applyEvent('discharge', amount, ref ?? -1);
    }

    givesanity(amount: number, ref?: number) {
        this.#applyEvent('givesanity', amount, ref ?? -1);
    }

    takesanity(amount: number, ref?: number) {
        this.#applyEvent('takesanity', amount, ref ?? -1);
    }
    ...
    removeFromInventory(cardRef: number): number {
        ...
        if (!isCanceled && updatedEvent.type === 'removecard') {
            ...
            afterScript();
            return updatedEvent.value.cardRef;
        }

        return -1;
    }

    attackDamage(amount: number, targetRef: number, usingCardRef: number) {
